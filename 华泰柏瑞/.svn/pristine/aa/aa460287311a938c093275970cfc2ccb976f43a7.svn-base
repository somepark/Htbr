...
//是否处理跳跃的连线
@property (nonatomic,assign) BOOL canJumpedCenterCircle;
 ...
 *  @param arrow 三角形箭头
 */
- (instancetype)initWithType:(CircleViewType)type clip:(BOOL)clip arrow:(BOOL)arrow solid:(BOOL)solid
{
    if (self = [super init]) {
        // 解锁视图准备
        self.type = type;
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        //默认处理跳跃的连线
        [self setCanJumpedCenterCircle:YES];
        // 解锁视图准备
        [self lockViewPrepare];
    }
    return self;
...
-(void)lockViewPrepare{
    ...
    if (self.type == CircleViewTypeLogin) {
        [self setCenter:CGPointMake([UIScreen mainScreen].bounds.size.width/2, CircleViewCenterY-35)];
    }else{
        [self setCenter:CGPointMake([UIScreen mainScreen].bounds.size.width/2, CircleViewCenterY + 35)];
    }
...
-(void)layoutSubviews{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(PCCircle *circle, NSUInteger idx, BOOL *stop) {
        [circle setSolid:self.solid];
    }];
    
    // 遍历子控件，改变其是否有箭头
    [self.subviews enumerateObjectsUsingBlock:^(PCCircle *circle, NSUInteger idx, BOOL *stop) {
        [circle setArrow:self.arrow];
    }];
    ...
    [self.subviews enumerateObjectsUsingBlock:^(PCCircle *circle, NSUInteger idx, BOOL *stop) {
        ...
        if (CGRectContainsPoint(circle.frame, point)) {
            if ([self.circleSet containsObject:circle]) {
                ...
                if (self.arrow) {
                    //每次计算每个圆的方向
                    [self calAnleJumpedCircle];
                }
                if(!self.canJumpedCenterCircle){
                    // move过程中的连线（包含跳跃连线的处理）
                    [self calconnectTheJumpedCircle];
                }
...
#pragma mark -不跳过跳跃中间间断的circle
-(void)calconnectTheJumpedCircle{
